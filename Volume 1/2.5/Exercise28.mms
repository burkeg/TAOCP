...
1H		LDO		k,L,:dynNode:KVAL		get k
		SET		(last+1),k
		SET		(last+2),L
		PUSHJ		last,:buddyk
		SET		P,last				P <- buddy_k(L)
		CMP		:t,k,:m
		BZ		:t,3F				if k=m skip to S3
		LDB		:t,P,:dynNode:TAG
		BZ		:t,3F				if TAG(P)=9 skip to S3
		LDB		:t,P,:dynNode:KVAL
		CMP		:t,:t,k
		BNZ		:t,3F				if TAP(P)=1 and KVAL(P)≠k
;	  	S2	    	[Combine with buddy.]
2H		LDO		:t,P,:dynNode:LINKF		get LINKF(P)
		LDO		last,P,:dynNode:LINKB		get LINKB(P)
		STO		:t,last,:dynNode:LINKF		LINKF(LINKB(P)) <- LINKF(P)
		LDO		:t,P,:dynNode:LINKB		get LINKB(P)
		LDO		last,P,:dynNode:LINKF		get LINKF(P)
		STO		:t,last,:dynNode:LINKB		LINKB(LINKF(P)) <- LINKB(P)
		ADD		k,k,1
		CMPU		:t,P,L
		BNN		:t,1B				if P>=L skip to S1
		SET		L,P				L <- P
;	  	S3	    	[Put on list.]
3H		SET		:t,1
		STB		:t,L,:dynNode:TAG		TAG(L) <- 1
		LDA		AVAILk,:AVAIL
		MUL		:t,k,16
		ADD		AVAILk,:t,AVAILk		AVAILk = LOC(AVAIL[k])
		LDO		P,AVAILk,:availNode:AVAILF	P <- AVAILF[k]
		STO		P,L,:dynNode:LINKF		LINKF(L) <- P
		STO		P,L,:dynNode:LINKB		LINKB(L) <- P
		STB		k,L,:dynNode:KVAL		KVAL(L) <- k
		STO		AVAILk,L,:dynNode:LINKB		LINKB(L) <- LOC(AVAIL[k])
		STO		L,AVAILk,:availNode:AVAILF	AVAILF[k]
		PUT		:rJ,retaddr
		POP	    	1,0
	  	PREFIX      	:
